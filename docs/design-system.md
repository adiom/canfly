...
| `text-cf-accent` / `bg-cf-accent` | `#d52525` | `#d52525` | Красный акцент: логотип, ошибки, деструктивные действия |
...
### Воздушный слой (`cf-air-*`, `cf-live-*`)

Прозрачные поверхности для разделов, где страница должна дышать: глубина строится светом и тоном, а не рамками и красным контрастом. Впервые применён на персонажах (`/characters/**`). Токены глобальные — переиспользовать на других разделах можно и нужно, не дублируя.

| Tailwind-класс | Светлая тема | Тёмная тема | Назначение |
|---|---|---|---|
| `bg-cf-air-surface` | `rgba(255,255,255,.62)` | `rgba(244,239,229,.05)` | Стекло поверх фона |
| `bg-cf-air-surface-2` | `rgba(255,255,255,.82)` | `rgba(244,239,229,.08)` | Поднятая поверхность, активная пилюля |
| `border-cf-air-line` | `rgba(255,255,255,.85)` | `rgba(244,239,229,.10)` | Волосяная линия-свет вместо бордера |
| `bg-cf-air-accent` | `#6a9ab8` | `#6a9ab8` | CF-002 canfly — **только заливка и точки** |
| `text-cf-air-accent-ink` | `#3d6e8c` | `#9db5c8` | CF-018 zenith — **акцентный текст и ссылки** |
| `text-cf-live-on` | `#3f8d7e` | `#75f1d9` | Персонаж отвечает сам |
| `text-cf-live-slow` | `#427f95` | `#63c0d9` | Отвечает не всегда |
| `text-cf-live-quiet` | `#7e8a86` | `#d9e2e4` | Молчит |

**Правила:**
- **`--cf-air-accent` — заливка, `--cf-air-accent-ink` — текст.** `#6a9ab8` на кремовом фоне даёт 2.6:1 и не годится для текста; зенит `#3d6e8c` — 4.9:1, проходит AA
- Готовые утилиты вместо россыпи arbitrary-значений: `.cf-glass` (заливка + `blur(20px)` + линия + мягкая тень), `.cf-glass-2` (поднятая, без тени), `.cf-air-field` (фиксированный ambient-фон страницы: три радиальных пятна под плёнкой `--cf-bg`)
- Тени описаны один раз в `--cf-air-shadow`; в компонентах свои `shadow-[...]` не писать
- Радиусы воздушного слоя: `rounded-3xl` для панелей и карточек, `rounded-full` для пилюль, аватаров и кнопок
- Типографика: `font-black uppercase` остаётся только у `h1` и eyebrow'ов, метки и чипы — `font-medium` + `tracking-[0.18em]`. Не больше двух весов на экран
- Сигнал присутствия рисовать только компонентом `components/character-presence.tsx` — точка и метка красятся одним токеном, поэтому метка обязана проходить AA (отсюда приглушённые значения в светлой теме)

### Палитра VVVVV (`/vvvvv`)

Лендинг литературной среды — единственное согласованное исключение из правила «только `cf-*`». VVVVV подан как самостоятельный бренд внутри экосистемы canfly, поэтому у него своя палитра в scoped-блоке `.vvvvv-scope` (`app/globals.css`). Правило «hex только в `globals.css`» при этом соблюдено — в компонентах хардкода нет.

| Переменная | Значение | Источник | Назначение |
|---|---|---|---|
| `--vv-void` | `#111210` | CF-004 `void` | Фон страницы |
| `--vv-page` | `#161714` | между CF-004 и CF-006 | Лист разворота |
| `--vv-ink` | `#f4efe5` | CF-003 `manuscript` | Основной текст |
| `--vv-ink-2` | `#cec8bb` | CF-003, приглушённый | Текст фрагмента |
| `--vv-ink-3` | `#8b8478` | CF-003, служебный | Метки, номера страниц |
| `--vv-ochre` | `#dcb463` | CF-016 `orpiment` | Единственный акцент |
| `--vv-rule` | `rgba(244, 239, 229, 0.1)` | CF-003 под 10% | Линии, корешок |

**Правила:**
- Применять через arbitrary-значения: `bg-[var(--vv-void)]`, `text-[var(--vv-ink)]`
- Блок `.vvvvv-scope` намеренно **не** зависит от `.dark` — страница остаётся в темноте void при любой выбранной посетителем теме
- Красный `cf-accent` на VVVVV не используется: акцент бренда — охра CF-016
- Класс `dark` на корневой обёртке `app/vvvvv/page.tsx` — это не запрещённый префикс `dark:`, а переключение `cf-*` в тёмные значения ради `SiteFooter`. Убирать нельзя: иначе футер ляжет светлой полосой на void
- Контраст на `--vv-void`: охра 9.7:1, `--vv-ink-3` 5.1:1 — обе выше AA
- Типографика VVVVV тоже своя: Cormorant Garamond (заголовки), EB Garamond (текст), Geist Mono (служебное). Приёмы из раздела 2 (`font-black uppercase`) там не применяются
...
- `any` в TypeScript
- `text-cf-air-accent` для текста — это заливочный тон, для надписей есть `text-cf-air-accent-ink`
- Красный `cf-accent` как акцент раздела: он оставлен логотипу, ошибкам и удалению

Единственное исключение по цветам — палитра VVVVV (раздел 1): свои токены `--vv-*` в scoped-блоке `.vvvvv-scope`. Все hex при этом остаются в `app/globals.css`, так что правило выше соблюдено.
